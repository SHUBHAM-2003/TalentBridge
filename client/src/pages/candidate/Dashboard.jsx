...
import { FileText, Clock, CheckCircle, Calendar, Briefcase, ArrowRight, User } from 'lucide-react'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setApplications(data.data.profile.applications || [])
  ...
  const stats = [
    { label: 'Applications Sent', value: applications.length, icon: FileText, color: 'bg-blue-50 text-blue-600' },
    { label: 'Under Review', value: applications.filter(a => a.status === 'UNDER_REVIEW').length, icon: Clock, color: 'bg-yellow-50 text-yellow-600' },
    { label: 'Interviews', value: applications.filter(a => a.status === 'INTERVIEW_SCHEDULED').length, icon: Calendar, color: 'bg-purple-50 text-purple-600' },
    { label: 'Offers', value: applications.filter(a => a.status === 'HIRED').length, icon: CheckCircle, color: 'bg-emerald-50 text-emerald-600' }
  ]

  const completion = profile ? Math.round(([profile.fullName, profile.phone, profile.bio, profile.skills?.length, profile.resumeUrl].filter(Boolean).length / 5) * 100) : 0

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold">Welcome, {profile?.fullName || user?.email}!</h1>
              ...
              {loading ? <Skeleton className="h-20 w-full" /> : applications.length === 0 ? (
                ...
                <div className="space-y-3">
                  {applications.slice(0, 5).map(app => {
                    const statusBadge = APPLICATION_STATUSES.find(s => s.value === app.status)
                    return (
                      <div key={app.id} className="flex items-center justify-between p-4 border rounded-lg">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusBadge?.color}`}>{statusBadge?.label}</span>
