...
export default function AdminCompanies() {
  ...
  useEffect(() => { fetch() }, [])
  const fetch = async () => {
    try { const { data } = await api.get('/admin/companies'); setCompanies(data.data.companies || []) } catch (e) { console.error(e) }
    setLoading(false)
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      if (editCompany) { await api.put(`/admin/companies/${editCompany.id}`, form); toast.success('Updated') }
      else { await api.post('/admin/companies', form); toast.success('Created') }
      setShowModal(false); fetch()
    } catch (e) { toast.error('Failed') }
  }
  const deleteCompany = async (id) => { if (confirm('Delete this company?')) { try { await api.delete(`/admin/companies/${id}`); toast.success('Deleted'); fetch() } catch (e) { toast.error('Failed') } } }
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="text-left p-4 font-medium text-sm">Jobs</th>
                ...
                {loading ? (
                  ...
                  companies.map(c => (
                    <tr key={c.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-4">{c._count?.jobs || 0}</td>
                      <td className="p-4">
                        <div className="flex gap-1">
                          <button onClick={() => openEdit(c)} className="p-2 hover:bg-gray-100 rounded"><Edit className="h-4 w-4" /></button>
                          <button onClick={() => deleteCompany(c.id)} className="p-2 hover:bg-gray-100 rounded text-red-600"><Trash2 className="h-4 w-4" /></button>
                        </div>
                      </td>
                    </tr>
                  ))
